...
const ListadoUsuarios = () => {
  ...
  const handleUpdateSharedcount = async (id, newValue) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `https://ecommerce-9558.onrender.com/users/${id}`,
        {
          ...
          body: JSON.stringify({ sharedcount: newValue }),
        }
      );

      if (!response.ok) {
        throw new Error('Error al actualizar sharedcount');
      }

      alert('Límite de productos actualizado correctamente');
      setUsuarios((prev) =>
        prev.map((u) => (u.id === id ? { ...u, sharedcount: newValue } : u))
      );
    } catch (error) {
      alert(error.message);
    }
        ...
        {loading ? (
          ...
          usuarios.map((usuario) => (
            <div key={usuario.id} className="listadoUsuarios__details">
              ...
              <p>Limite de Productos: {usuario.sharedcount}</p>

              {/* Campo para editar el límite */}
              <input
                type="number"
                value={usuario.sharedcount}
                onChange={(e) =>
                  setUsuarios((prev) =>
                    prev.map((u) =>
                      u.id === usuario.id
                        ? { ...u, sharedcount: parseInt(e.target.value) }
                        : u
                    )
                  )
                }
              ...
              <button
                onClick={() =>
                  handleUpdateSharedcount(usuario.id, usuario.sharedcount)
                }
              >
                Guardar límite
              </button>
...
//       alert('Sharedcount actualizado correctamente');
//       setUsuarios((prev) =>
//         prev.map((u) => (u.id === id ? { ...u, sharedcount: newValue } : u))
//       );
//     } catch (error) {
...
//               <button
//                 onClick={() =>
//                   handleUpdateSharedcount(usuario.id, usuario.sharedcount + 1)
//                 }
//               >
//                 Incrementar Sharedcount
//               </button>
//               <button
//                 className="cambiarPasswordBoton"
//                 onClick={() => handleChangePassword(usuario.id)}
